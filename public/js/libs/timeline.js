    ...
    if (google && google.visualization &&
        ...
        for (var row = 0, rows = data.getNumberOfRows(); row < rows; row++) {
            items.push(this.createItem({
				...
				'rangeStyle':((cols.rangeStyle != undefined)   ? data.getValue(row, cols.rangeStyle)   : undefined),
        ...
        if (!params.moved && !params.zoomed) {
            ...
            if (params.target === this.dom.items.deleteButton) {
                ...
                if (params.itemIndex != undefined) {
                    //if (!this.isSelected(params.itemIndex)) { // Comented by holmes
                        this.selectItem(params.itemIndex);
                        this.trigger('select');
                    //}
                }
...
links.Timeline.prototype.onMouseWheel = function(event) {
    if (!this.options.zoomable || !event.ctrlKey)
        return;
...
links.Timeline.Item = function (data, options) {
    if (data) {
		...
		this.rangeStyle = data.rangeStyle; // holmes!
...
links.Timeline.ItemRange.prototype.createDOM = function () {
	...
	// Content box holmes
    var divContent = document.createElement("DIV");
	divContent.className = "timeline-label";
	divBox.appendChild(divContent);

    // range box
    var divRange = document.createElement("DIV");
    divRange.className = "timeline-event-content";
    divBox.appendChild(divRange);
...
links.Timeline.ItemRange.prototype.updateDOM = function () {
    ...
    if (divBox) {
		...
		divBox.firstChild.innerHTML = this.content;
		if (this.rangeStyle){
			divBox.childNodes[1].innerHTML = this.rangeStyle;
		}
        ...
        // add item specific class name when provided
        if (this.className) {
			links.Timeline.addClassName(divBox.childNodes[1], this.className);
			//divBox.childNodes[1].className = this.className;				//holmes
        }
...
links.Timeline.prototype.getItem = function (index) {
    ...
    itemData.start = item.start != null && item.start != undefined ? new Date(item.start.valueOf()) : null;
    if (item.end) {
...
links.Timeline.prototype.changeItem = function (index, itemData, preventRender) {
    ...
    var newItem = this.createItem({
		...
		'rangeStyle': itemData.hasOwnProperty('rangeStyle') ? itemData.rangeStyle : oldItem.rangeStyle,
